##759. Employee Free Time
# We are given a list schedule of employees,
# which represents the working time for each employee.
# Each employee has a list of non-overlapping Intervals,
#  and these intervals are in sorted order.
# Return the list of finite intervals representing common,
# positive-length free time for all employees, also in sorted order.
# (Even though we are representing Intervals in the form [x, y],
# the objects inside are Intervals, not lists or arrays. For example,
# schedule[0][0].start = 1, schedule[0][0].end = 2,
# and schedule[0][0][0] is not defined).
#  Also, we wouldn't include intervals like [5, 5] in our answer, as they have zero length.
# Example 1:

# Input: schedule = [[[1,2],[5,6]],[[1,3]],[[4,10]]]
# Output: [[3,4]]
# Explanation: There are a total of three employees, and all common
# free time intervals would be [-inf, 1], [3, 4], [10, inf].
# We discard any intervals that contain inf as they aren't finite.


"""
# Definition for an Interval.
class Interval:
    def __init__(self, start: int = None, end: int = None):
        self.start = start
        self.end = end
"""


class Solution:
    def employeeFreeTime(self, schedule):
        i = 0
        free_time = []
        while i < len(schedule):
            free_time.append(self.getFreeTime(schedule[i]))
            i += 1
        j = 0
        tmp = free_time[0]
        while j < len(free_time) - 1:
            tmp = self.merge(tmp, free_time[j + 1])
            j += 1
        tmp.pop(0)
        tmp.pop()
        return tmp

    def merge(self, time1, time2):
        i = 0
        j = 0
        n = len(time1)
        m = len(time2)
        res = []
        while i < n and j < m:
            if time1[i][1] <= time2[j][0]:
                i += 1
                continue
            if time1[i][0] >= time2[j][1]:
                j += 1
                continue
            res.append([max(time1[i][0], time2[j][0]), min(time1[i][1], time2[j][1])])
            if time1[i][1] < time2[j][1]:
                i += 1
            else:
                j += 1
        return res

    def getFreeTime(self, schedule):
        res = []
        n = len(schedule)
        i = 0
        while i < n:
            if i == 0:
                res.append([-float("inf"), schedule[i].start])
            if i == n - 1:
                res.append([schedule[i].end, float("inf")])
                i += 1
                continue
            res.append([schedule[i].end, schedule[i + 1].start])
            i += 1
        return res
schedule =  [[[1,3],[6,7]],[[2,4]],[[2,5],[9,12]]]
s= Solution()
print(s.employeeFreeTime(schedule))
